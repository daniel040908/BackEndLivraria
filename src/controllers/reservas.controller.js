...
export async function criarReserva(req, res) {
    try {
        ...
        WHERE livro_id = ? 
        AND status = 'ativa' 
        AND data_devolucao > NOW()`, [livro_id]
        );
        ...
        await db.query(
            "INSERT INTO reservas (usuario_id, livro_id, data_retirada, data_devolucao) VALUES (?, ?, ?, ?)", [usuario_id, livro_id, data_retirada, data_devolucao]
        );
